...
for epoch in range(EPOCHS):
    running_loss = 0.0
    model.train()
    for id, example in enumerate(tokenized_train['input_ids'][0:25]):
        #model.initHidden()
        ...
        train_counter.append(id*(epoch+1))
        print(f"Epoch: {epoch+1} | example: {id} | loss: {loss.item()}")

    print(f"Loss after epoch {epoch}: {running_loss}")
